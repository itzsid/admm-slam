...
/* Test this
#if USE_BIASED_PRIOR_FACTOR == 1
              sub_graph.add(BiasedPriorFactor<Pose2, Vector>(key, pose_z, -u_k, noiseModel::Isotropic::Variance(3,2/rho_)));
#else
              Matrix A = -Matrix::Identity(3,3);
              Pose2 pose_x = sub_initial.at<Pose2>(key);
              Vector b = - (Pose2::Logmap(pose_x.between(pose_z)) + u_k );
              sub_graph.add(LinearContainerFactor( JacobianFactor(key, A, b, noiseModel::Isotropic::Variance(3,2/rho_)) ) );
#endif
*/
