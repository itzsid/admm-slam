/**
 * @file    testBiasedBetweenFactor.cpp
 * @brief  
...
TEST(BiasedBetweenFactor, Rot3) {
  ...
  Vector bias = gtsam::Vector3(0.5,0.5,0.5);

  BiasedBetweenFactor<Rot3, Vector> factor(R(1), R(2), measured, bias, Isotropic::Sigma(3, 0.05));
  Vector actual = factor.evaluateError(R1, R2);
  Vector expected = Rot3::Logmap(measured.inverse() * R1.between(R2)) + bias;
